...
class NewGoodsItemView: UIView {
    ...
    fileprivate var platform = UILabel()
    ...
    override init(frame: CGRect) {
        ...
        let platform = UIImageView(image: self.icon)
        ...
        self.platform.textColor = .white
        self.platform.textAlignment = .center
        self.platform.font = FontSize(10)
        self.platform.layer.cornerRadius = 3
        self.platform.layer.masksToBounds = true
        self.platform.configureLayout { (layout) in
            layout.isEnabled = true
            layout.width = YGValue(32)
            layout.height = YGValue(16)
            layout.marginRight = 4
        }
        goodsName.addSubview(self.platform)
    ...
    func getDataToView(_ data: goodsItem){
        ...
        var icon1 = UIImage.init(named: "taobao")
        if data.shoptype == "B" {
            icon1 = UIImage.init(named: "tianmao")
            typeLabel.text = "天猫"
