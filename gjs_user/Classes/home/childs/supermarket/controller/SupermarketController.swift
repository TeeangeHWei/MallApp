...
class SupermarketController: UIViewController, WKNavigationDelegate, UIWebViewDelegate {
    
    var isCancel = false
    let supermarketWebView = WKWebView(frame: CGRect(x:0 ,y: headerHeight, width: kScreenW, height: kScreenH - headerHeight))
    ...
    let webAddress = "https://chaoshi.m.tmall.com/"
    var goodsId = ""
    var couponUrl = ""
...
//            delegate.window?.viewWithTag(101)?.removeFromSuperview()
    ...
    func webView(_ webView: WKWebView, didCommit: WKNavigation!){
        ...
        if urlStr!.contains("item.htm") {
            ...
            var endIndex = urlStr!.positionOf(sub: "&addressId=", backwards: false)
...
//            IDToast.id_show(msg: "已授权", success: .fail)
    ...
    func getRid (_ key : String) {
        ...
        success:{(res,data) in
            ...
            print("个人相信",UserDefaults.getInfo())
        },
        error:{
