//
//  GoodShotListViewController.swift
//  gjs_user
//
...
class GoodShotListViewController: ViewController,UIScrollViewDelegate {
    var isCancel = false
    let body = UIScrollView()
    var shotlisttitle : String?
        // 标题分类id
        var themeId : Int?
        var allHeight = 0
        var offset = 0
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        getData(refresh: false)
        let navView = customNav(titleStr: shotlisttitle!, titleColor: kMainTextColor)
        ...
        func getData (refresh: Bool) {
            ...
            AlamofireUtil.post(url: "/ddk/public/ddkThemeGoodsSearch", param: ["themeId": self.themeId!], success: { (res, data) in
                if self.isCancel {
                    return
                }
                self.offset += 10
                let classifyGoodsList : [pddGoodsItem] = pddGoodsItemList.deserialize(from: data.description)!.goodsList!
                for item in classifyGoodsList {
                    if #available(iOS 11.0, *) {
                        let goodsItem = PddGoodsCard(frame: CGRect(x: 0, y: self.allHeight, width: Int(kScreenW), height: 100),data: item, nav: self.navigationController!)
