...
class ShareDialog: UIView {
    ...
    convenience init(frame: CGRect, itemData: RecommendItem, goodsData: DetailData, qrUrl: String, checkIndex: Int) {
        ...
        let url = URL(string: itemData.itempic![checkIndex])!
        ...
        goodsName.text = goodsData.itemtitle!
        let titleH = getLabHeigh(labelStr: goodsData.itemtitle!, font: FontSize(14), width: kScreenW - 100) + 2
        goodsName.numberOfLines = 0
        goodsName.id_canCopy = true
        goodsName.textColor = UIColor.black
        if goodsData.shoptype == "C" {
        ...
        // 原价
        var platform = "淘宝"
        if goodsData.shoptype! == "B" {
            platform = "天猫"
        }
        let oldPrice = UILabel()
        oldPrice.text = "\(platform)价：\(itemData.itemprice!)元"
        ...
        couponMoney.text = "¥\(itemData.couponmoney!)"
        ...
        // 销量
        let sales = UILabel()
        sales.text = "\(goodsData.itemsale!)人已买"
        ...
        newLabel2.text = "¥\(itemData.itemendprice!)"
