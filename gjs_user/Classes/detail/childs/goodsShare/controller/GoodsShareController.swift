...
@available(iOS 11.0, *)
class GoodsShareController: ViewController {
    ...
    override func viewDidLoad() {
        ...
        // 预计奖励
        let commission = Commons.strToDou(self.goodsInfo!.tkmoney!) * Commons.getScale()
...
//        shareLink = "https://www.ganjinsheng.com/user/inviteShare?id=\(detailId!)&invite=\(UserDefaults.getInfo()["inviteCode"]!)&tbPwd=\(tPwd)"
        descTextStr =  "\(goodsInfo!.itemtitle!)\n【原价】：\(goodsInfo!.itemprice!)元\n【券后】：\(goodsInfo!.itemendprice!)元\n优惠下单：\n\(shortUrl)\n【复制打开手机淘宝】\n领取下单\(tPwd)\n【推荐语】：\(goodsInfo!.itemdesc!)"
        ...
        // 分享按钮组
        let shareBtns = UIView(frame: CGRect(x: 0, y: kScreenH - 120, width: kScreenW, height: 100))
...
//        setNav(titleStr: "创建分享", titleColor: kMainTextColor, navItem: navigationItem, navController: navigationController)
    }
    func setNav(){
...
//        detailId = 596349108854
    ...
    @objc func poster (_ btn : UIButton) {
        let recommendData = RecommendItem.init(data: self.goodsInfo!)
        let shareDialog = ShareDialog(frame: CGRect(x: 0, y: 0, width: kScreenW, height: kScreenW), itemData: recommendData, goodsData: self.goodsInfo!, qrUrl: shortUrl, checkIndex: self.checkIndex)
