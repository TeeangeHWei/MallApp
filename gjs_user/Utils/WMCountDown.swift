...
final class WMCountDown {
    ...
    fileprivate func calculateCurrentDate() {
        ...
        if  currentDate!.second! == 0
            ...
            let EndTimeStamp10 = timeToTimeStamp(time: "9:59:59", inputFormatter: "HH:mm:ss")
            let EndTimeStamp12 = timeToTimeStamp(time: "11:59:59", inputFormatter: "HH:mm:ss")
            let EndTimeStamp15 = timeToTimeStamp(time: "14:59:59", inputFormatter: "HH:mm:ss")
            let EndTimeStamp20 = timeToTimeStamp(time: "19:59:59", inputFormatter: "HH:mm:ss")
            let EndTimeStamp24 = timeToTimeStamp(time: "23:59:59", inputFormatter: "HH:mm:ss")
            if dateStamp < EndTimeStamp10 {
                resume()
                start(with: nil, end: "\(dateYMD) 10:00:00")
            }else if dateStamp < EndTimeStamp12{
                resume()
                start(with: nil, end: "\(dateYMD) 12:00:00")
            }else if dateStamp < EndTimeStamp15{
                resume()
                start(with: nil, end: "\(dateYMD) 15:00:00")
            }else if dateStamp < EndTimeStamp20{
                resume()
                start(with: nil, end: "\(dateYMD) 20:00:00")
            }else if dateStamp < EndTimeStamp24{
                resume()
                start(with: nil, end: "\(dateYMD) 23:59:59")
            }
